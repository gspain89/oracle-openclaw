...
| 차트 | **Apache ECharts** v5 (CDN) | GPU 렌더링, 그라데이션 fill, glow 이펙트, elastic 애니메이션. Chart.js 대비 시각적 완성도 우위 |
...
| 프론트엔드 디자인 | **"Data Observatory" 컨셉** | 딥 스페이스 다크 테마, 점 그리드 오버레이, 틸/민트+앰버 악센트 |
...
├── docs/                                 # 연구 문서 + 이 계획서
│   ├── openclaw-glm5-benchmark-research.md
...
│   │   ├── run-korean-bench.sh           # 한국어 벤치마크 실행
│   │   ├── run-all.sh                    # 전체 오케스트레이터 (모델 순회)
│   │   └── deploy-results.sh             # git push → Pages 트리거
│   ├── python/
│   │   ├── normalize.py                  # 벤치마크 출력 → 통합 JSON
│   │   └── generate_leaderboard.py       # leaderboard.json 생성
│   └── korean-bench/
│       ├── tasks/                         # 한국어 태스크 정의 (15개)
│       ├── manifest.json
│       └── evaluator.py                   # 채점 엔진
├── results/
...
│   │   └── korean/
│   └── normalized/
│       ├── leaderboard.json               # 리더보드 데이터 (현재 샘플)
│       ├── runs.json                      # 전체 실행 기록
│       └── history.json                   # 시계열 추적
├── site/                                  # Astro 정적 사이트
│   ├── package.json
...
│       └── pages/
│           ├── index.astro                # 메인 리더보드 (테이블+바차트)
│           ├── compare.astro              # A/B 모델 비교 (레이더 차트)
│           ├── cost.astro                 # 비용 효율 (산점도)
│           ├── history.astro              # 점수 변화 추적 (시계열)
│           └── korean.astro               # 한국어 벤치마크
...
- `history.json`에 타임스탬프+점수 append
- `history.astro` 시계열 차트 활성화
...
**컨셉**: "Data Observatory" — 우주 관측소 대시보드

| 요소 | 설정 |
|------|------|
| 배경 | #040712 (딥 스페이스), 도트 그리드 + 듀얼 라디얼 그라데이션 글로우 |
| 폰트 (제목) | Bricolage Grotesque (가변 폰트, wt 800) |
| 폰트 (데이터) | JetBrains Mono (모노스페이스) |
| 폰트 (본문) | Noto Sans KR |
| 1차 악센트 | #00e8c4 (시안/민트) |
| 2차 악센트 | #ffbe2e (앰버/골드) |
| 3차 악센트 | #a78bfa (퍼플) |
| 카드 스타일 | Glass morphism — `backdrop-filter: blur(16px)`, 반투명 배경, mask-composite 그라데이션 보더 |
| 순위 뱃지 | 1위 골드+글로우, 2위 실버, 3위 브론즈 |
| 점수 바 | CSS 그라데이션 fill + glow shadow (시안, 퍼플, 앰버 3단계) |
| 차트 | Apache ECharts v5 — GPU 렌더링, LinearGradient/RadialGradient 채우기, elasticOut 이징 |
| 네비게이션 | 스티키 상단, 블러 백드롭, 활성 탭 그라데이션 밑줄 |
| 애니메이션 | fade-up 진입(stagger), bar-fill 점수, ECharts elastic 바 (120ms 딜레이/바), symbol-pulse 로고 |

**페이지 구성**:
- **리더보드** (index): 통계 카드 4개 + 정렬 가능 테이블 + ECharts 바 차트 (순위별 그라데이션 색상)
- **모델 비교** (compare): 2개 모델 드롭다운 → ECharts 레이더 차트 (그라데이션 area fill) + 지표 바 + 상세 테이블
- **비용 효율** (cost): ECharts 산점도 (RadialGradient 포인트, 크기=점수 인코딩) + 비용 효율 순위 리스트
- **히스토리** (history): Phase 6 활성화 예정. 현재 빈 상태 + 스냅샷 그리드 + 로드맵
- **한국어** (korean): 3개 카테고리 카드 (5태스크씩) + 채점 방식 3종 설명 + 독자적 가치 설명
