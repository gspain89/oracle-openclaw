...
# ── 결과 요약 출력 ── PinchBench가 output-dir에 <NNNN>_<model>.json 생성
# maxdepth 1로 제한해 artifacts/*/meta.json 을 결과 파일로 오인하지 않도록 한다
RESULT_FILE=$(find "$RUN_OUTPUT_DIR" -maxdepth 1 -type f -name "*.json" 2>/dev/null | head -1)
